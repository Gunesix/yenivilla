<template>
  <AdminLayout :breadcrumbs="breadcrumbs">
    <div class="max-w-4xl mx-auto space-y-6">
      <!-- Header -->
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Yeni Fiyatlandırma Ekle</h1>
        <p class="text-gray-600">Villa için dönemsel fiyatlandırma oluşturun</p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="flex justify-end gap-4">
          ...
          >
            {{ processing ? 'Kaydediliyor...' : 'Fiyatlandırma Oluştur' }}
          </button>
        </div>
      </form>
    </div>
...
const breadcrumbs = [
  ...
  { name: 'Yeni Fiyatlandırma', current: true }
]
...
const form = useForm({
  ...
  start_date: '',
  end_date: '',
  price_per_night: props.selectedVilla?.price_per_night || 0,
  minimum_stay: 1,
  season_name: '',
  notes: '',
  is_active: true
...
const submit = () => {
  form.post(route('admin.villa-pricing.store'))
}
