<template>
  <AdminLayout :breadcrumbs="breadcrumbs">
    <div class="max-w-4xl mx-auto space-y-6">
      <!-- Header -->
      <div>
        <h1 class="text-2xl font-bold text-gray-900">Yeni Uygunluk Kaydı</h1>
        <p class="text-gray-600">Villa için uygunluk durumu oluşturun</p>
      </div>
      ...
      <form @submit.prevent="submit" class="space-y-6">
        ...
        <div class="bg-white rounded-lg shadow-lg p-6 border border-gray-100">
          <div class="flex items-center mb-6">
            ...
            <div>
              ...
              <p class="text-sm text-gray-600">Uygunluk durumunun geçerli olacağı tarih aralığını belirtin</p>
            </div>
          </div>
          ...
          <div v-if="dateRangeInfo" class="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
        ...
        <div class="flex justify-end gap-4">
          ...
          >
            {{ processing ? 'Kaydediliyor...' : 'Uygunluk Oluştur' }}
          </button>
        </div>
      </form>
    </div>
...
const props = defineProps({
  villas: Array,
  selectedVilla: Object,
  errors: Object
})

const breadcrumbs = [
  { name: 'Villa Uygunluk Yönetimi', href: route('admin.villa-availability.index') },
  { name: 'Yeni Uygunluk', current: true }
]

const form = useForm({
  villa_id: props.selectedVilla?.id || '',
  start_date: '',
  end_date: '',
  is_available: true,
  reason: '',
...
const dateRangeInfo = computed(() => {
  ...
  return `Seçilen tarih aralığı ${diffDays} gün içermektedir`
})

const submit = () => {
  form.post(route('admin.villa-availability.store'))
}
</script>